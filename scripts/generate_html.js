...
function generateListHTML(json) {
	...
	const startHTML =
		`<li data-required="[REQUIRED_SEX]"><span class="condensed">${name}</span><ul>`.replace(
			'[REQUIRED_SEX]',
			requiredSex
		);
	...
	for (let variant in variants) {
		...
		const itemIdFor =
			typeName + '-' + name.replaceAll(' ', '_') + '_' + itemName.replaceAll(' ', '_');
			...
			for (let jdx = 1; jdx < 10; jdx++) {
				...
				if (layerDefinition !== undefined) {
					if (sexIdx === 0) {
						...
						dataFiles +=
							'data-preview_row=' + previewRow + ' data-layer_' + jdx + '_zpos=' + zPos + ' ';
